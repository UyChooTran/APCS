...
public class SimpleImage
{
    ...
    {//right now its only going diagonally
        //pixelBlock = blockSize;
        int runX = width/ blockSize;
        int runY = height/ blockSize;
        int startY = 0;
        int startX = 0;
        ...
        for (int c = 0; c < runY; c++)
        {
            for ( int i = 0; i < runX; i++)
            {
                for (int j = startX; j < endX; j++)
                {
                    ...
                    {
                        System.out.println("k" + k + " J " + j + " endY" + endY + "endX " + endX);
                        switch (image [j][k])
                        {
                            case 0: 
                            r++;
                ...
                count(r,b,g,startX,startY,endX,endY);
                startX = endX;
                // startY = endY;
    ...
    public void count(int r, int b, int g, int startX, int startY, int endX, int endY)
    {
        if (r > b && r > g)
        { for( int j = startX; j< endX; j++)
            {
        ...
        { 
            for( int j = startX; j< endX; j++)
            {
                for (int k = startY; k < endY; k++)
                {
                    System.out.println("Row = " + j + " Col = " + k + "EndY = " + endY); 
                    image [j][k] = 1;
                }
            }
        }
